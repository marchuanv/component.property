...
export class PropertiesReference extends Reference {
    ...
    */
    constructor(context) {
        ...
        if (!super.get()) {
            const { properties } = typeRegister.get();
            super.set({
                properties: properties.map(propReg => {
    ...
    */
    get(property, propertyType) {
        ...
        const { properties } = super.get();
        const prop = properties.find(x => x.name === propertyName && x.targetClass === propertyType);
        if (!prop) {
    ...
    */
    set(property, propertyType) {
        const propertyName = Object.keys(property)[0];
        const { properties, events } = super.get();
        const prop = properties.find(x => x.name === propertyName && x.targetClass === propertyType);
        if (!prop) {
            throw new Error(`${propertyName} property no found.`);
        }
