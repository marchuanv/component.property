...
export class PropertyReference extends Reference {
    ...
    */
    constructor(context) {
        if (new.target !== ReferenceProperty) {
            throw new Error(`${ReferenceProperty.name} is a sealed class.`);
        }
        super(context);
        const { propertyTypeRegister } = context.get();
        const { type, propertyName, isGetter, isSetter } = propertyTypeRegister.get();
        if (!super.get()) {
            super.set({
                setterCallback: null,
                propertyName,
